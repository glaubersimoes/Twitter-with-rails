...
Quando /^eu preencho "([^\"]*)" com "([^\"]*)"$/ do |field, value|
  Quando %{I fill in ("#{field}", :with => "#{value}"}
end
...
Quando /^eu anexo o arquivo em "([^\"]*)" a "([^\"]*)"$/ do |path, field|
  When %{I attach the file "#{Rails.root}/#{path}" to "#{field}"}
end
...
Entao /^segura ai por "([^\"]*)" segundos$/ do |tempo|
  sleep(tempo.to_i)
end

Então /^a página deve conter um javascript apontando para "([^"]*)"$/ do |url_js|
  page.should have_xpath("//script[contains(@src, '#{url_js}')]")
end

Então /^a página deve conter um stylesheet apontando para "([^"]*)"$/ do |url_css|
  page.should have_xpath("//link[contains(@href, '#{url_css}')]")
end
